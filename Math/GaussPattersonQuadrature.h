...
namespace Grok
{
	extern const QuadraturePoint* gauss_patterson_rule[10];


	template <typename TYPE>
	void GaussPattersonQuadrature(int level, Vector<QuadratureNode<TYPE, 1> >& quadrature_node) throw(MemoryException)
	{
		Assert((level >= 1) && (level <= 10));

		try
		{
			const QuadraturePoint* rule = gauss_patterson_rule[level - 1];

			int number_of_points = (1 << level) - 1;
	...
	template <typename TYPE>
	void GaussPattersonQuadrature(int level, Vector<QuadratureNode<TYPE, 2> >& quadrature_node) throw(MemoryException)
	{
		Assert((level >= 1) && (level <= 10));
	...
	template <typename TYPE>
	void GaussPattersonQuadrature(int level, Vector<QuadratureNode<TYPE, 3> >& quadrature_node) throw(MemoryException)
	{
		Assert((level >= 1) && (level <= 10));
